...
format_title.set_bg_color('#1ccccd')   #定义format_title对象单元格背景颜色为
                                       #'#cccccc'的格式
format_title.set_color('#3z33az')
...
format_txt=workbook.add_format()
format_txt.set_border(1)
format_txt.set_color('#2222h0')
#下面分别以行或列写入方式将标题、业务名称、流量数据写入起初单元格，同时引用不同格式对象
worksheet.write_row('A1',title,format_title)  
worksheet.write_column('A2', buname,format)
worksheet.write_row('B2', data[0],format_txt)
worksheet.write_row('B3', data[1],format_txt)
worksheet.write_row('B4', data[2],format_txt)
worksheet.write_row('B5', data[3],format_txt)
worksheet.write_row('B6', data[4],format_txt)
