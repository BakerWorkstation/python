/* 
   ...
   This code has been improved by Juan M. Casillas in order to provide
   more functionality. This code is based on Tim Potter's work, and the
   original idea and Copyrights are under his property.
...
static PyObject *py_smb_connect(PyObject *self, PyObject *args)
{
					...
					PyObject *kw)
{
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kw, "s|s", kwlist, 
				   &called_name, 
				   &calling_name))
    return NULL;
...
static PyObject *py_smb_negprot(PyObject *self, PyObject *args)
{
  cli_state_object *cli = (cli_state_object *)self;
  BOOL result;
				      ...
				      PyObject *kw)
{
  ...
  result = cli_session_setup(cli->cli, username, 
...
static PyObject *py_smb_tconx(PyObject *self, PyObject *args)
{
  cli_state_object *cli = (cli_state_object *)self;
  ...
  result = cli_send_tconX(cli->cli, service, 
			  strequal(service, "IPC$") ? "IPC" : 
			  "?????", "", 1);
				       ...
				       PyObject *kw)
{
  ...
  uint32 desired_access, file_attributes = 0, 
    ...
    create_disposition = OPENX_FILE_EXISTS_OPEN, 
    create_options = 0;
  int result;

  /* Parse parameters */

  if (!PyArg_ParseTupleAndKeywords(args, kw, "si|iiii", 
				   kwlist, &filename, &desired_access,
				   &file_attributes, &share_access, 
				   &create_disposition,
				   &create_options))
    return NULL;

  result = cli_nt_create_full(cli->cli, filename, 0, 
...
static PyObject *py_smb_read(PyObject *self, PyObject *args, PyObject *kw)
{
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kw, "i|ii", 
				   kwlist, &fnum, &offset, &size))
    return NULL;
...
static PyObject *py_smb_mkdir(PyObject *self, PyObject *args)
{
  ...
  if ( !cli_resolve_path( "", cli->cli, dir, &targetcli, targetname )) {
    PyErr_SetString(PyExc_RuntimeError, cli_errstr(cli->cli));
    return NULL;
  }
...
static PyObject *py_smb_exists(PyObject *self, PyObject *args)
{
  ...
  if ( !cli_resolve_path("", cli->cli, 
			 wdir, &targetcli, targetpath ) ) {
    PyErr_SetString(PyExc_RuntimeError, cli_errstr(targetcli));
    return NULL;
  }
...
static PyObject *py_smb_pwd(PyObject *self, PyObject *args)
{
  ...
  return Py_BuildValue("s", cli->curdir);
}


static PyObject *py_smb_chdir(PyObject *self, PyObject *args)
{
  ...
  pstrcpy(saved_dir, cli->curdir);

  if (*p == '\\')
    pstrcpy(cli->curdir,p);
  else
    pstrcat(cli->curdir,p);

  if ((cli->curdir[0] != '\0') && 
      (*(cli->curdir+strlen(cli->curdir)-1) != '\\')) {
    pstrcat(cli->curdir, "\\");
  }
  
  dos_clean_name(cli->curdir);
  pstrcpy( dname, cli->curdir );
  pstrcat(cli->curdir,"\\");
  dos_clean_name(cli->curdir);
  
  if ( !cli_resolve_path( "" , cli->cli, dname, &targetcli, targetpath ) ) {
    goto error_ex;
  }
...
static void do_list_helper(const char *mntpoint, file_info *f, 
			   const char *mask, void *state)
{

  cli_state_object * cli = (cli_state_object *)state;
  ...
  PyDict_SetItemString(dict,"name", Py_BuildValue("s", f->name));
  PyDict_SetItemString(dict,"size", Py_BuildValue("f", (double)f->size));
  PyDict_SetItemString(dict,"isdir", 
		       Py_BuildValue("i", (f->mode & aDIR ? 1: 0)));
  PyList_Append(cli->listdir_q, dict);
  return;
}


static PyObject *py_smb_listdir(PyObject *self, PyObject *args)
{
  ...
  if ( cli_resolve_path("", cli->cli, 
    ...
    if (cli_list(targetcli, targetpath,
		 attribute, do_list_helper, (void *)cli) == -1) {
      // list empty 
      // PyErr_SetString(PyExc_RuntimeError, cli_errstr(targetcli));
      return cli->listdir_q;
    }
  }
...
static PyObject *py_smb_filesize(PyObject *self, PyObject *args)
{
  ...
  if (!cli_qfileinfo(cli->cli, fnum, 
		     NULL, &fsize, NULL, NULL,
		     NULL, NULL, NULL) &&
      !cli_getattrE(cli->cli, fnum, NULL, &fsize, NULL, NULL, NULL)) {
    PyErr_SetString(PyExc_RuntimeError, "getattrib failed (fsize)");
    return NULL;
  }


  return PyInt_FromLong(fsize);
}
...
static PyObject *py_smb_diskavail(PyObject *self, PyObject *args)
{
  ...
  if ( !cli_resolve_path( base, cli->cli, cur_dir, &targetcli, targetpath ) ) {
    PyErr_SetString(PyExc_RuntimeError, cli_errstr(cli->cli));
    return NULL;
  }
  ...
  return Py_BuildValue("(iii)", total, avail, bsize);
}



static PyObject *py_smb_write(PyObject *self, PyObject *args)
{
  cli_state_object *cli = (cli_state_object *)self;
  int fnum, offset=0, size=0;
  ssize_t result;
  ...
  if (!PyArg_ParseTuple(args, "iiis#", 
			&fnum, &offset, &size, &ptr))
    return NULL;

  result = cli_write(cli->cli, fnum, 0, ptr, (off_t) offset, (size_t) size);

  if (result==-1) {
    PyErr_SetString(PyExc_RuntimeError, "write failed result=-1");
    return NULL;
  }
				    ...
				    PyObject *kw)
{
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kw, "iO", 
				   kwlist, &fnum, &py_secdesc))
    return NULL;
...
/*
 *
 * Object initialization and method definition
 *
 */

static void py_cli_state_dealloc(PyObject *);
static PyObject *py_cli_state_getattr(PyObject *, char *);
...
static PyMethodDef smb_hnd_methods[] = {
  ...
  { "session_request", 
    (PyCFunction)py_smb_session_request, 
  ...
  { "session_setup", 
    (PyCFunction)py_smb_session_setup,
    METH_VARARGS | METH_KEYWORDS, 
    "Session setup" 
  },

  { "tconx", 
    (PyCFunction)py_smb_tconx,
    METH_VARARGS , 
    "Tree connect" 
  },
  ...
  { "nt_create_andx", 
    (PyCFunction)py_smb_nt_create_andx,
    METH_VARARGS | METH_KEYWORDS, 
    "NT Create&X" },

  { "open", 
    (PyCFunction)py_smb_open,
    METH_VARARGS | METH_KEYWORDS,
    "Open a file\n"
    "\n"
  ...
  /* 
     jmc hack some methods are defined here, in order to improve  the code 
  */

  { "rmdir", 
    (PyCFunction)py_smb_rmdir,
    METH_VARARGS, 
    "Deletes a directory" 
  },

  { "mkdir", 
    (PyCFunction)py_smb_mkdir,
    METH_VARARGS, 
    "Makes a directory. If parent is true, the intermediate dirs are created" 
  },

  { "exists", 
    (PyCFunction)py_smb_exists,
    METH_VARARGS, 
  ...
  { "chdir", 
    (PyCFunction)py_smb_chdir,
    METH_VARARGS, 
    "Changes to the given dir" 
  },


  { "list", 
    (PyCFunction)py_smb_listdir,
    METH_VARARGS, 
    "List the given dir contents" 
  },


  { "diskavail", 
    (PyCFunction)py_smb_diskavail,
    METH_VARARGS , 
    "Return in bytes the space available for this share" 
  },


  { "write", 
    (PyCFunction)py_smb_write,
    METH_VARARGS, 
    "Write a chunck of data inside a remote file" 
  },


  { "filesize", 
    (PyCFunction)py_smb_filesize,
    METH_VARARGS, 
    "Return in bytes the size occupied by file" },


  { "close", 
    (PyCFunction)py_smb_close,
    METH_VARARGS, 
    "Closes an 'opened' file " 
  },

  { "unlink", 
    (PyCFunction)py_smb_unlink,
    METH_VARARGS, 
  ...
  { "query_secdesc", 
    (PyCFunction)py_smb_query_secdesc,
    METH_VARARGS, 
    "Query security descriptor" 
  },

  { "set_secdesc", 
    (PyCFunction)py_smb_set_secdesc,
    METH_VARARGS | METH_KEYWORDS, 
...
/*
 * Method dispatch tables (for module)
 */

static PyMethodDef smb_methods[] = {

  { "connect", 
    (PyCFunction)py_smb_connect, 
    METH_VARARGS ,
    "Connect to a host" 
  },
  ...
  { "setup_logging", 
    (PyCFunction)py_setup_logging, 
    METH_VARARGS | METH_KEYWORDS, 
    "Set up debug logging.\n"
    "\n"
    "Initialises Samba's debug logging system.  "
    "One argument is expected which\n"
    "is a boolean specifying whether debugging is "
    "interactive and sent to stdout\n"
    "or logged to a file.\n"
    "\n"
    "Example:\n"
    "\n"
...
{
  /* handler to dispatch methods for our object conn */
...
void initsmb(void)
{
  ...
  cli_state_type.tp_new = PyType_GenericNew;
